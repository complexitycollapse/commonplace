...
describe('SingleZettelSchneider.zettel', () => {
  it('returns a single zettel if the clip is a box', () => {
    ...
    expect(SingleZettelSchneider(box, [l]).zettel()).toHaveLength(1);
  });
  ...
  it('assigns a key to the returned zettel if a keyPrefix is passed', () => {
    ...
    expect(SingleZettelSchneider(box, [l], "abc").zettel()[0].key).toBe("abc.0");
  });
  ...
  it('does not assign a key to the returned zettel if no keyPrefix is passed', () => {
    ...
    expect(SingleZettelSchneider(box, [l], undefined).zettel()[0].key).toBe(undefined);
  });

  it('returns a single zettel if the clip is a span and there are no links', () => {
    let s = Span("origin", 0, 10);

    expect(SingleZettelSchneider(s, []).zettel()).toHaveLength(1);
  });
  ...
  it('attaches an endset to a box that overlaps with it', () => {
    ...
    let zettel = SingleZettelSchneider(box, [l]).zettel();
  ...
  it('attaches an endset to a span that overlaps with it', () => {
    ...
    let zettel = SingleZettelSchneider(s, [l]).zettel();
  ...
  it('assigns all links that overlap the zettel', () => {
    ...
    let zettel = SingleZettelSchneider(s, [l1, l2]).zettel();
  ...
  it('will assign consecutive sub-keys to all created zettel', () => {
    ...
    let zettel = SingleZettelSchneider(s, [l1, l2], "1").zettel();
...
describe('ManyZettelSchneider.zettel', () => {
  it('will split a singleton list of clips the same as SingleZettelSchneider does', () => {
    let s = Span("origin", 1, 20);
    let l1 = makeSpanLink({ clipLists: [[Span("origin", 2, 18)]] });
    let l2 = makeSpanLink({ clipLists: [[Span("origin", 11, 4)]] });

    let zettel = ManyZettelSchneider([s], [l1, l2]).zettel();
  ...
  it('returns a concatenated list of all zettel that all clips are split into, respecting different origins', () => {
    let s1 = Span("origin1", 0, 10), s2 = Span("origin2", 100, 20);
    let l1 = makeSpanLink({ clipLists: [[Span("origin1", 5, 5)]] });
    let l2 = makeSpanLink({ clipLists: [[Span("origin2", 105, 20)]] });

    let zettel = ManyZettelSchneider([s1, s2], [l1, l2]).zettel();

    expect(zettel).toHaveLength(4);
    expect(zettel[0]).not.hasEndset(l1, 0);
    expect(zettel[0]).not.hasEndset(l2, 0);
    expect(zettel[1]).hasEndset(l1, 0);
    expect(zettel[1]).not.hasEndset(l2, 0);

    expect(zettel[2]).not.hasEndset(l1, 0);
    expect(zettel[2]).not.hasEndset(l2, 0);
    expect(zettel[3]).not.hasEndset(l1, 0);
    expect(zettel[3]).hasEndset(l2, 0);
  });

  it('returns a concatenated list of all zettel that all clips are split into when clips have the same origin', () => {
    let s1 = Span("origin", 0, 10), s2 = Span("origin", 100, 20);
    let l1 = makeSpanLink({ clipLists: [[Span("origin", 5, 5)]] });
    let l2 = makeSpanLink({ clipLists: [[Span("origin", 105, 20)]] });
